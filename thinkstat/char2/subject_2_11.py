...
sum_a = sum(yy_a)
pmf_a = [float(i) / sum_a for i in yy_a]

# 生成模拟人群1000名
data_simulate = [1000 * i for i in pmf_a]
# 删除所有怀孕周数小于39的[0: 38]
# data_39 = data_simulate[39:]
for i in range(39):
    data_simulate[i] = 0
print data_simulate
# 计算剩余怀孕周期的pmf，这就是一个条件pmf
sum_39 = sum(data_simulate)
pmf_conditional = [float(i) / sum_39 for i in data_simulate]
# 计算x=39时，条件pmf的值
print pmf_conditional[39]
